...
To also not make the managers too big, they use specific policy related helper classes: `PolicyParser`, `PolicyInterpreter` and `PolicyService`. These classes provide the basic functionality for the managers to implement their functions. 

### Policy Helper Classes

This section handles the implementation of the `PolicyParser`, `PolicyInterpreter` and `PolicyService`. They do not follow the generic type system as used in the rest of the controller-structure.
...
The `PolicyService` is used to make the required UMA server calls. The main functionality is to insert an atomic rule to add a new permission for a subject to a target, and the function to delete a permission for a subject to a target. 
...
The loama implementation has always used a refresh-function to make sure that the data of the editted resource is up to date. This is not necessary in the new implementation, where we can just use the response of the PATCH as the new state for the policy. PATCH returns this state right after the update. This would mean that we could just pass through the response, instead of making separate calls. 
...
Because most of the work is done in the `PolicyService` and `PolicyInterpreter`, there is no big implementation in the managers. The `createPermissions` and `deletePermissions` functions are one-liners, and the `editPermissions` is not actually needed in this implementation. 
...
- `enablePermissions`/`disablePermissions`: We have removed the old implementation, and have no need to find a way to implement this the odrl way.
