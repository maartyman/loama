...
import { AccessRequestMessage, BaseSubject, Index, IndexItem, Permission, RequestResponseMessage, ResourceAccessRequestNode, ResourceOwnerAsset, ResourcePermissions, SubjectPermissions } from "../types";
...
export interface IController<T extends Record<keyof T, BaseSubject<keyof T & string>>> {
    ...
    */
    ...
    getManageableAssets(options?: ManageableAssetOptions): Promise<ResourceOwnerAsset[]>;
...
export interface ManageableAssetOptions {
    include?: string[];
}

export type ManageableAssetEvent =
    | { type: 'snapshot'; assets: ResourceOwnerAsset[] }
    | { type: 'asset-created' | 'asset-updated'; asset: ResourceOwnerAsset }
    | { type: 'asset-deleted'; asset: ResourceOwnerAsset };
