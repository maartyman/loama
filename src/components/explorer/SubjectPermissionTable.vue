    ...
    <div v-if="podStore.selectedEntry">
        <DataTable :value="podStore.selectedEntry?.permissionsPerSubject ?? []">
            ...
            <Column>
                ...
                <template #body="slotProps">
                    <LoCheck v-if="slotProps.data.permissions?.includes(Permission.Delete)" />
                </template>
            </Column>
            <Column header="">
                <template #body="slotProps">
        ...
        <Drawer :visible="!!selectedSubject" @update:visible="handleSubjectDrawerClose" header="Edit subject"
            ...
            <div v-if="selectedSubject">
                ...
                <div>
                    ...
                    <LoSwitch :id="Permission.Delete"
                        :default-value="selectedSubject.permissions.includes(Permission.Delete)" :disabled="updating"
                        @update:checked="checked => handleSubjectPermissionUpdates(checked, Permission.Delete)">
                        {{ Permission.Delete }}
                    </LoSwitch>
                </div>
            </div>
