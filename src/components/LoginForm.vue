...
import { getClientIdUrl, getRedirectUrl, trustflowsAuth } from '@/lib/trustflowsAuth';
import { normalizeIdentityProviderUrl } from '@/lib/identityProvider';
...
const login = async () => {
    ...
    try {
        const issuer = normalizeIdentityProviderUrl(solidPodUrl.value || defaultSolidPodUrl);
        const searchParams = new URLSearchParams(location.search)
        const nextPath = searchParams.get("next") ?? "home"

        await trustflowsAuth.login(issuer, getClientIdUrl(), getRedirectUrl(nextPath))
    } catch (error) {
        console.error('Failed to start Solid OIDC login.', error);
